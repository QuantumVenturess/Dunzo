...
- (void) fadeBackground 
{
  void (^animations) (void) = ^(void) {
    ...
    self.layer.borderColor        = [UIColor grayAlpha: opacity].CGColor;
    UIColor *fadedColor           = [UIColor grayAlpha: opacity];
...
- (void) restoreBackground 
{
  void (^animations) (void) = ^(void) {
    self.backgroundColor          = [UIColor whiteAlpha: 0.6];
    self.layer.borderColor        = [UIColor gray].CGColor;
    horizontalBar.backgroundColor = [UIColor gray];
    verticalBar.backgroundColor   = [UIColor gray];
  };
  [UIView animateWithDuration: 0.15 delay: 0
    options: UIViewAnimationOptionCurveLinear animations: animations
      completion: nil];
